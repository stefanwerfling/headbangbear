...
export class Lang_EN implements LangDefine {
    ...
    private readonly _content: { [index: string]: string } = {
        ...
        settings: 'Settings',
        settings_help: 'Configure where Headbangbear pulls tracks from. Changes take effect on the next library scan.',
        settings_library_source: 'Library source',
        settings_source_local: 'Local files (config.json → library.rootDir)',
        settings_source_jellyfin: 'Jellyfin server',
        settings_source_help: 'Switching to Jellyfin will become active in a follow-up release. For now this is stored as the preferred source.',
        settings_jellyfin_help: 'Personal API key (Admin → Dashboard → API Keys) and the user GUID whose audio library you want to expose.',
        ...
        settings_jellyfin_user_id_placeholder: 'leave blank to auto-discover',
        settings_jellyfin_user_id_help: 'Find at Dashboard → Server → Users → click user → URL contains <code>userId=…</code>. Or leave blank — the test will pick the first user the API key sees and fill this field for you.',
        settings_jellyfin_test: 'Test connection',
        settings_jellyfin_testing: 'Probing server…',
        settings_save: 'Save',
        ...
        library_scan_banner_title: 'Library scan in progress',
        library_scan_banner_progress: 'Progress',
        library_scan_banner_done: 'Library scan complete',
        library_scan_banner_failed: 'Library scan failed',
